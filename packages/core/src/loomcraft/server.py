...
def create_router(
    ...
    *,
    ...
    def broker_for(session: Session) -> ToolBroker:
        ...
        if existing is not None and not turns.is_busy(session.id):
            return existing
        return ToolBroker(session, registry, limits=limits)
    ...
    async def catalog() -> dict[str, Any]:
        return {
            "capabilities": [
                item.contract() for item in registry.capabilities.values()
            ],
            "workflows": [item.contract() for item in registry.workflows.values()],
        }
    ...
    async def turn(
        ...
        broker = ToolBroker(session, registry, limits=limits)
...
def create_app(
    ...
    *,
    ...
    app.include_router(
        create_router(
            store, registry, agent_factory, prefix=prefix, limits=limits, manager=manager
        )
    )

...
from .engine import Engine, Run, graph_from_capability, graph_from_workflow
from .errors import (
    ActionBudgetError,
    AwaitingInputsError,
    ExecutionBusyError,
...
class ToolBroker:
    ...
    async def dispatch(self, name: str, payload: Mapping[str, Any] | None = None) -> ToolResponse:
        ...
        except Exception as exc:  # noqa: BLE001 - stable model-facing boundary
    ...
    def _update_step(self, payload: dict[str, Any]) -> ToolResponse:
        ...
        if step["kind"] not in plan_module.AGENT_REPORTABLE_KINDS:
    ...
    async def _run_capability(self, payload: dict[str, Any]) -> ToolResponse:
        ...
        plan_module.ensure_step_startable(
            current, step_id, kind="capability", capability=capability_id
        )

...
[![tests: 346 passing](https://img.shields.io/badge/tests-346%20passing-34d399?style=flat-square&labelColor=0b1120)](#testing)
...
A `review` may instead bind an explicitly review-scoped capability (a
`review.*` runner or a capability tagged `review`). In that case it is completed
by `run_capability`, and `update_step` cannot bypass the registered review.
...
├── tools.py      Core + opt-in extension tools as JSON Schema, in 4 dialects
...
├── ai.py         Chat/Responses/JSONL/Codex provider adapters
├── plan_executor.py  Whole-plan adapter that reuses Engine
├── runtime.py    Runtime + FastAPI assembly convenience
...
The package also includes a normalized `AIProvider`,
JSONL/Codex adapters, `execute_plan`, research-objective metadata, bounded table
inspection, and knowledge-provider hooks. They are opt-in extensions over the
same broker and event contract, not a second engine.
...
python -m pytest -q                                         # 285 Python tests (234 core + 51 compatibility)
cd packages/renderer && npm ci && npm test                   # 61 renderer tests
```
...
The published documentation is limited to user, integration, and extension
guides; internal working notes are kept out of the release tree.
